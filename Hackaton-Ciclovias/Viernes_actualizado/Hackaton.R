...
# 2010: sin columna id_usuario
rec_2010 <- read_csv("Hackaton-cs-de-datos-e250/csv-source/recorridos-realizados-2010.csv") %>%
  summarise(anio = 2010, registros_reales = n(), tipo_dato = "Recorridos")


# 2011: sin columna id_usuario
rec_2011 <- read_csv("Hackaton-cs-de-datos-e250/csv-source/recorridos-realizados-2011.csv") %>%
  summarise(anio = 2011, registros_reales = n(), tipo_dato = "Recorridos")
...
rec_2012
# Unir todos los años
reales <- bind_rows(rec_2010, rec_2011, rec_2012, rec_2013, rec_2014, usr_2015, usr_2016, usr_2017)
reales <- reales |> mutate( estaciones = c(NA, NA, 54, 62, 65, 118, 198, 390)                             )
...
# #
# ciclistas_anuales_csv <- read_csv("Hackaton-cs-de-datos-e250/csv-source/volumen-ciclistas-anuales.csv")
# ciclistas_anuales <- ciclistas_anuales_csv |> mutate( anio = "aÃ±o" ) |>  group_by(anio, cantidad_ciclistas)

conteo <- read_csv("Hackaton-cs-de-datos-e250/csv-source/Conteocicilistas-GCBA.csv")
...
resumen <- comparacion %>%
  ...
  select(anio, viajes_anuales_est, registros_reales, tipo_dato, cobertura_pct, estaciones)
resumen
...
for (i in 1:nrow(resumen)) {
  f <- resumen[i, ]
  cat(sprintf(
    "Año %d | Estimado: %s | Registrado: %s (%s) | Cobertura: %s% | Estaciones: %s %\n",
    f$anio,
    ...
    format(f$estaciones,   big.mark = ".", scientific = FALSE)
  ))
}
...
grafico <- ggplot(datos_grafico, aes(x = factor(anio), y = cantidad, fill = tipo)) +
  geom_line(stat = "identity", position = "dodge", size = 0.7) +
...
ggsave("grafico_comparacion_viajes.png", grafico
