...
graph_baratheon_endo_undir <- as_undirected(graph_baratheon_endo, mode = "collapse")
summary(graph_baratheon_endo_undir)
...
centr_degree(graph_baratheon_endo_undir, mode = "total")$centralization
## [1] 0.04282795
...
centr_clo(graph_baratheon_endo_undir, mode = "total")$centralization
## [1] 1.138423
...
centr_eigen(graph_baratheon_endo_undir, directed = FALSE)$centralization
## [1] 0.8787532
...
graph_baratheon_endo_undir_degree <- igraph::degree(graph_baratheon_endo_undir, mode = "all") # mode puede ser all, out, in, total. "all" y "total" son sinónimos. Este argumento es ignorado en gráficos no dirigidos.

#standardized by number of nodes
graph_baratheon_endo_undir_degree_std <- graph_baratheon_endo_undir_degree / (vcount(graph_baratheon_endo_undir) - 1)

node_degree <- data.frame(degree = graph_baratheon_endo_undir_degree,
                          degree_std = graph_baratheon_endo_undir_degree_std) %>%
    tibble::rownames_to_column() # ordena estos datos recién calculados en un df

baratheon_characters <- left_join(baratheon_characters, node_degree, by = c("name" = "rowname")) #junta en una sola columna llamada "rowname" el nombre y apellido de cada personaje
...
closeness <- igraph::closeness(graph_baratheon_endo_undir, mode = "total")

#standardized by number of nodes
closeness_std <- closeness / (vcount(graph_baratheon_endo_undir) - 1)

node_closeness <- data.frame(closeness = closeness,
                             closeness_std = closeness_std) %>%
    tibble::rownames_to_column()

baratheon_characters <- left_join(baratheon_characters, node_closeness, by = c("name" = "rowname"))
...
betweenness <- igraph::betweenness(graph_baratheon_endo_undir, directed = FALSE)

# standardize by number of node pairs
betweenness_std <- betweenness / ((vcount(graph_baratheon_endo_undir) - 1) * (vcount(graph_baratheon_endo_undir) - 2) / 2)
...
edge_betweenness <- igraph::edge_betweenness(graph_baratheon_endo_undir, directed = FALSE) # creo un objeto igraph

data.frame(edge = attr(E(graph_baratheon_endo_undir), "vnames"), # creo un df con 2 cols: edge (vnames es el nombre del vértice de los edges) y betweenness de cada uno de esos edges ???
...
plot(graph_baratheon_endo_undir,
     layout = layout,
     vertex.label = gsub(" ", "\n", V(graph_baratheon_endo_undir)$name),
     vertex.shape = V(graph_baratheon_endo_undir)$shape,
     vertex.color = V(graph_baratheon_endo_undir)$color,
     ...
     edge.arrow.size = 0.5,
     edge.color = E(graph_baratheon_endo_undir)$color,
     edge.lty = E(graph_baratheon_endo_undir)$lty)
...
# In contrast to the shortest path between two nodes, we can also calculate the longest path, or diameter:
    ...
    diameter(graph_baratheon_endo_undir, directed = FALSE)
...
# This, we can also plot:
    ...
    graph_baratheon_endo_undir_diameter <- graph_baratheon_endo_undir
node_diameter <- get_diameter(graph_baratheon_endo_undir_diameter,  directed = FALSE)

V(graph_baratheon_endo_undir_diameter)$color <- scales::alpha(V(graph_baratheon_endo_undir_diameter)$color, alpha = 0.5)
V(graph_baratheon_endo_undir_diameter)$size <- 2

V(graph_baratheon_endo_undir_diameter)[node_diameter]$color <- "red"
V(graph_baratheon_endo_undir_diameter)[node_diameter]$size <- 5

E(graph_baratheon_endo_undir_diameter)$color <- "grey"
E(graph_baratheon_endo_undir_diameter)$width <- 1

E(graph_baratheon_endo_undir_diameter, path = node_diameter)$color <- "red"
E(graph_baratheon_endo_undir_diameter, path = node_diameter)$width <- 5

plot(graph_baratheon_endo_undir_diameter,
     layout = layout,
     vertex.label = gsub(" ", "\n", V(graph_baratheon_endo_undir_diameter)$name),
     vertex.shape = V(graph_baratheon_endo_undir_diameter)$shape,
     vertex.frame.color = "gray",
     vertex.label.color = "black",
     vertex.label.cex = 0.8,
     edge.arrow.size = 0.5,
     edge.lty = E(graph_baratheon_endo_undir_diameter)$lty)
legend("topleft", legend = c("Node color:", as.character(color_vertices$house), NA, "Edge color:", as.character(colors_edges$type)), pch = 19,
       col = c(NA, color_vertices$color, NA, NA, colors_edges$color), pt.cex = 5, cex = 2, bty = "n", ncol = 1)
...
# We can calculate the transitivity or ratio of triangles to connected triples for the whole network:
    ...
    transitivity(graph_baratheon_endo_undir, type = "global")

## [1] 0.2850679

# Or for each node:

    transitivity <- data.frame(name = V(graph_baratheon_endo_undir)$name,
                               transitivity = transitivity(graph_baratheon_endo_undir, type = "local")) %>% # la transitividad es 1 o 0 para cara personaje
...
page_rank <- page_rank(graph_baratheon_endo_undir, directed = FALSE)
...
adjacency <- as.matrix(as_adjacency_matrix(graph_baratheon_endo_undir))
...
degree_diag <- diag(1 / igraph::degree(graph_baratheon_endo_undir))
...
# We can find the eigenvector centrality scores with:
    ...
    eigen_centrality <- igraph::eigen_centrality(graph_baratheon_endo_undir, directed = FALSE)
...
#igraph::dyad_census(graph_baratheon_endo_undir)
...
#igraph::triad_census(graph_baratheon_endo_undir)
...
# The largest group of nodes ín this network is three, i.e. all parent/child relationships. Therefore, it does not really make sense to plot them all, but we could plot and color them with:
    ...
    vcol <- rep("grey80", vcount(graph_baratheon_endo_undir))

# highlight first of largest cliques
vcol[unlist(largest_cliques(graph_baratheon_endo_undir)[[1]])] <- "red"
...
# We can also look for groups within our network by clustering node groups according to their edge betweenness:
    ...
    ceb <- cluster_edge_betweenness(graph_baratheon_endo_undir)
modularity(ceb)

## [1] 0.8359884

plot(ceb,
     graph_baratheon_endo_undir,
     layout = layout,
     vertex.label = gsub(" ", "\n", V(graph_baratheon_endo_undir)$name),
     vertex.shape = V(graph_baratheon_endo_undir)$shape,
     vertex.size = (V(graph_baratheon_endo_undir)$popularity + 0.5) * 5,
     vertex.frame.color = "gray",
     vertex.label.color = "black",
     vertex.label.cex = 0.8)
...
# Or based on propagating labels:
    ...
    clp <- cluster_label_prop(graph_baratheon_endo_undir)
